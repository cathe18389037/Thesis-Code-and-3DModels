...
const char *TOPIC_Light= "SeedBed/light";  // Topic to subcribe to
const char *TOPIC_TEMP = "SeedBed/temp";
const char *TOPIC_Moist= "SeedBed/moist"; 
const char *TOPIC_Uv= "SeedBed/uv"; 
const char *TOPIC_Pump= "esp32/pump"; 

const char* mqttServer = "192.168.137.189";  // Broker address
...
static void refreshValue(unsigned long interval){
  ...
  if(now-past>interval){
    ...
    client.publish(TOPIC_Uv, String(getUVIndex()).c_str() );
   }
}
...
//===================================================================
void callback(char* TOPIC_Pump, byte* message, unsigned int length) {
  Serial.print("Arrived on TOPIC_Pump: "); Serial.print(TOPIC_Pump);  Serial.print(".Message: ");  
  String msg;
  ...
  if(String(TOPIC_Pump) == "esp32/pump") {
    Serial.print("pump: ");
    if(msg == "on"){
